...
def agent_parcoureur(point,children={}):
    ...
    while  old_chidren:
        ...
        for j in intersections:
            pygame.draw.circle(display,"green",j,2)
        for t in old_chidren:
            pygame.draw.circle(display,color,t,5)
        pygame.display.flip()
...
relations = dict(map(lambda x:(x,1),intersections))
...
i = 0
RRRR = list(relations.keys())

while run:
    if i < len(RRRR):
        for o in agent_parcoureur(RRRR[i]):
            pass
    i += 1
...
def find_closest(point,direction):
    ...
    first_white = None
    swap = False
    while old_generation:
        for child in old_generation:
            new_childs = dict(map(lambda x: (x,1),list(filter(lambda t: (not children.get(t)) and (not new_generation.get(t)) and (not old_generation.get(t)) and 0<=t[0]<img.size[0] and 0<=t[1]<img.size[1],[somme_int(child,direction),somme_int(child,produit(-1,direction))]))))
    ...
    if not first_black or not first_white:
        return point
...
ABCD = pygame.transform.scale2x(pygame.transform.scale2x(pygame.transform.scale2x(pygame.transform.scale2x(imagee))))
run = True
...
def skeletonizer(point1,point2):
    ...
    changed = 0
    while not changed:
        display.blit(ABCD,(540 - 16*point1[0],360 - 16*point1[1]))
        print(old_sections,(point1,point2))

        changed = 1
        for section in old_sections:
            pygame.draw.rect(display,"red",(section[0][0] + 540 - 16*point1[0],section[0][1] + 360 - 16*point1[1],20,20))
            pygame.draw.rect(display,"red",(section[1][0] + 540 - 16*point1[0],section[1][1] + 360 - 16*point1[1],20,20))

            if section[0] == section[1] or is_neighbour(section[0],section[1]):
                continue
            if section[2] == 2:
            ...
            if gray[center[1]][center[0]] > 155:
                ...
                if not section[2]:
                    ...
                    new_sections.append([section[0],center,2])
                    new_sections.append([center,section[1],2])
    ...
    result = dict()
    joins = sum(list(map(lambda p:p[:2],old_sections)),start=[])
