...
class DrawObject:
    ...
    def save(self):
        with open("points.txt","wb") as file:
...
def analyser(point:POINT):
    ...
    DDD.pass_complexe(list(map(lambda x: x[0] if type(x) != str else None,right_scanner + left_scanner)))
    KKKKK.append((DDD.active.copy(),DDD.L))
